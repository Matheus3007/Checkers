...
class Board:  # Define the game board as an array of squares
    def __init__(self, debug=False):
        ...
        for row in range(
            constant.BOARD_SIZE
        ):  # Counts the amount of pieces for each player
    ...
    def DefineBoard(self):  # Creates a board for the game
    ...
    def DrawBoard(self, screen):
        ...
        for i in range(constant.BOARD_SIZE):
            for j in range(
                i % 2, constant.BOARD_SIZE, 2
            ):  # Draws the light squares on a dark background, beginning with the first square in even rows and in the second square in odd rows
                pygame.draw.rect(
                    screen,
                    constant.SECONDARY_COLOR,
                    (
                        i * constant.SQUARE_SIDE,
                        j * constant.SQUARE_SIDE,
                        constant.SQUARE_SIDE,
                        constant.SQUARE_SIDE,
                    ),
                )
    ...
    def GetMoves(self, piece):
        ...
        if piece.color == constant.P1_CENTER or piece.crowned:
            moves.update(
                self._LeftDiagonal(
                    row - 1, max(row - 3, -1), -1, piece.color, squareLeft
                )
            )
            moves.update(
                self._RightDiagonal(
                    row - 1, max(row - 3, -1), -1, piece.color, squareRight
                )
            )
        ...
        for r in range(start, stop, direction):
            ...
            if current == 0:
                ...
                if last:
                    ...
                    moves.update(
                        self._LeftDiagonal(
                            r + direction, row, direction, color, left - 1, skipped=last
                        )
                    )
                    moves.update(
                        self._RightDiagonal(
    ...
    def RemovePiece(self, pieces):
        ...
        if self.p1_pieces == 0:
            ...
            if (
                not self.debug
            ):  # If the game is not in debug mode, it will show the winner
